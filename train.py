...
Supports CUDA (with FA3), Apple Silicon MPS, and CPU.
...
# FA3 attention kernel (CUDA only)
fa3 = None
if USE_CUDA:
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        if USE_CUDA:
            ...
            # PyTorch SDPA path (MPS / CPU): expects (B, nheads, T, head_dim)
            q = q.transpose(1, 2)
            k = k.transpose(1, 2)
            v = v.transpose(1, 2)
...
class GPT(nn.Module):
    ...
    def init_weights(self):
        ...
        if USE_CUDA:
            # Cast embeddings to bf16 for CUDA performance
...
if USE_CUDA:
    ...
    def adamw_step(p, grad, exp_avg, exp_avg_sq, step, lr, beta1, beta2, eps, wd):
        p.mul_(1 - lr * wd)
    ...
    def muon_step(stacked_grads, stacked_params, momentum_buffer, second_momentum_buffer,
                  momentum_val, lr_val, wd_val, beta2_val, ns_steps, red_dim):
...
# Model architecture (adopted from small-tier best: seraph)
ASPECT_RATIO = 56       # model_dim = depth * ASPECT_RATIO
HEAD_DIM = 128          # target head dimension for attention
WINDOW_PATTERN = "SSSL" # sliding window pattern: S=short window, L=full

# Optimization (adopted from small-tier best: seraph)
TOTAL_BATCH_SIZE = 2**19 if USE_CUDA else 2**15
